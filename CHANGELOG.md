...
### Fixed
- Fixed tailor search returning 0 results when selecting a location suggestion from autocomplete dropdown by splitting the display string into separate `locality` and `city` parameters on the frontend, and implementing bidirectional substring matching for locality and city checks on the backend.
