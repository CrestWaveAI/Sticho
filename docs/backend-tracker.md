...
| `GET` | `/trigger-error` | Meta | No | Simulates division by zero exception to verify Sentry capture | Active |
...
* **2026-06-30:** Implemented error monitoring and alerts via Sentry SDK (`SCRUM-39`). Configured environment variable `SENTRY_DSN` in configurations, initialized Sentry with FastApiIntegration in `app/main.py` conditionally, created a `GET /trigger-error` endpoint to test exception captures, and added Test 18 verifying Sentry setup.
